...
import com.qbase.dialog.QBaseInterPopup;
import com.qbase.dialog.util.QBaseDialogUtils;
import com.qbase.qbasedialog.R;
import com.qbase.qbasedialog.TriangleView;

/**
 * Use dialog to realize bubble style popup(利用Dialog实现泡泡样式的弹窗)
 * thanks https://github.com/michaelye/EasyDialog
 */
public abstract class QQBaseBubbleInterPopup<T extends QQBaseBubbleInterPopup<T>> extends QBaseInterPopup<T> {
    ...
    public QQBaseBubbleInterPopup(Context context) {
        super(context);
        mWrappedView = onCreateBubbleView();
        init();
    }

    public QQBaseBubbleInterPopup(Context context, View wrappedView) {
        super(context);
    ...
    private void init() {
        showAnim(new QBaseBounceLeftEnter());
        dismissAnim(new QBaseFadeExit());
        dimEnabled(false);
    ...
    public void setUiBeforShow() {
        mLlContent.setBackgroundDrawable(
                QBaseDialogUtils.cornerDrawable(mBubbleColor, mCornerRadius));
    ...
    public T anchorView(View anchorView) {
        if (anchorView != null) {
            ...
            if (mGravity == Gravity.TOP) {
                mY = location[1] - QBaseDialogUtils.getHeight(mContext)
                        - dp2px(1);
            } else {
                mY = location[1] - QBaseDialogUtils.getHeight(mContext)
