 ...
 */
public class DialogActionSheet extends QBaseBottomTopDialog<DialogActionSheet> {
    /** ListView */
    ...
    private ArrayList<QBaseDialogMenuItem> mContents = new ArrayList<>();
    ...
    public DialogActionSheet(Context context, ArrayList<QBaseDialogMenuItem> baseItems, View animateView) {
        super(context, animateView);
        mContents.addAll(baseItems);
        init();
    }

    public DialogActionSheet(Context context, String[] items, View animateView) {
        super(context, animateView);
        mContents = new ArrayList<>();
        for (String item : items) {
            QBaseDialogMenuItem customBaseItem = new QBaseDialogMenuItem(item, 0);
            mContents.add(customBaseItem);
        }
        init();
    }

    public DialogActionSheet(Context context, BaseAdapter adapter, View animateView) {
        super(context, animateView);
        mAdapter = adapter;
        init();
    }
    ...
    public void setUiBeforShow() {
        ...
        mTvTitle.setBackgroundDrawable(QBaseDialogUtils.cornerDrawable(mTitleBgColor, new float[]{radius, radius, radius,
                radius, 0, 0, 0, 0}));
        ...
        mTvCancel.setBackgroundDrawable(QBaseDialogUtils.listItemSelector(radius, mLvBgColor, mItemPressColor, 1, 0));
        ...
        if (mIsTitleShow) {
            mLv.setBackgroundDrawable(QBaseDialogUtils.cornerDrawable(mLvBgColor, new float[]{0, 0, 0, 0, radius, radius, radius,
                    radius}));
        } else {
            mLv.setBackgroundDrawable(QBaseDialogUtils.cornerDrawable(mLvBgColor, radius));
        }
        ...
        mLv.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (mOnItemClick != null) {
                    mOnItemClick.onOperItemClick(parent, view, position, id);
                }
            }
        });
    ...
    public DialogActionSheet titleBgColor(int titleBgColor) {
        mTitleBgColor = titleBgColor;
        return this;
    }

    /** set title text(设置标题内容) */
    public DialogActionSheet title(String title) {
        mTitle = title;
        return this;
    }

    /** set titleHeight(设置标题高度) */
    public DialogActionSheet titleHeight(float titleHeight) {
        mTitleHeight = titleHeight;
        return this;
    }

    /** set title textsize(设置标题字体大小) */
    public DialogActionSheet titleTextSize_SP(float titleTextSize_SP) {
        mTitleTextSize = titleTextSize_SP;
        return this;
    }

    /** set title textcolor(设置标题字体颜色) */
    public DialogActionSheet titleTextColor(int titleTextColor) {
        mTitleTextColor = titleTextColor;
        return this;
    }

    /** enable title show(设置标题是否显示) */
    public DialogActionSheet isTitleShow(boolean isTitleShow) {
        mIsTitleShow = isTitleShow;
        return this;
    }

    /** set ListView background color(设置ListView背景) */
    public DialogActionSheet lvBgColor(int lvBgColor) {
        mLvBgColor = lvBgColor;
        return this;
    }

    /** set corner radius(设置圆角程度,单位dp) */
    public DialogActionSheet cornerRadius(float cornerRadius_DP) {
        mCornerRadius = cornerRadius_DP;
        return this;
    }

    /** set divider color(ListView divider颜色) */
    public DialogActionSheet dividerColor(int dividerColor) {
        mDividerColor = dividerColor;
        return this;
    }

    /** set divider height(ListView divider高度) */
    public DialogActionSheet dividerHeight(float dividerHeight_DP) {
        mDividerHeight = dividerHeight_DP;
        return this;
    }

    /** set item press color(item按住颜色) */
    public DialogActionSheet itemPressColor(int itemPressColor) {
        mItemPressColor = itemPressColor;
        return this;
    }

    /** set item textcolor(item字体颜色)* @return ActionSheetDialogQBaseQBottomBottomTop */
    public DialogActionSheet itemTextColor(int itemTextColor) {
        mItemTextColor = itemTextColor;
        return this;
    }

    /** set item textsize(item字体大小) */
    public DialogActionSheet itemTextSize(float itemTextSize_SP) {
        mItemTextSize = itemTextSize_SP;
        return this;
    }

    /** set item height(item高度) */
    public DialogActionSheet itemHeight(float itemHeight_DP) {
        mItemHeight = itemHeight_DP;
        return this;
    }

    /** set layoutAnimation(设置layout动画 ,传入null将不显示layout动画) */
    public DialogActionSheet layoutAnimation(LayoutAnimationController lac) {
        mLac = lac;
        return this;
    }

    /** set cancel btn text(设置取消按钮内容) */
    public DialogActionSheet cancelText(String cancelText) {
        mCancelText = cancelText;
        return this;
    }

    /** cancel btn text color(取消按钮文字颜色) */
    public DialogActionSheet cancelText(int cancelTextColor) {
        mCancelTextColor = cancelTextColor;
        return this;
    }

    /** cancel btn text size(取消按钮文字大小) */
    public DialogActionSheet cancelTextSize(float cancelTextSize) {
    ...
    class ListDialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            final QBaseDialogMenuItem item = mContents.get(position);
            ...
            float radius = dp2px(mCornerRadius);
            if (mIsTitleShow) {
                llItem.setBackgroundDrawable((QBaseDialogUtils.listItemSelector(radius, Color.TRANSPARENT, mItemPressColor,
